...
from pokemon_legacy.engine import pokemon_generator, item_generator

from pokemon_legacy.engine.bag.bag import BagV2
from pokemon_legacy.engine.battle.battle import Battle, BattleOutcome
from pokemon_legacy.engine.game_world.game_map import TallGrass
from pokemon_legacy.engine.game_world.game_obejct import PokeballTile
from pokemon_legacy.engine.pokedex.pokedex import Pokedex
from pokemon_legacy.engine.game_log.game_log import GameLog, GameEvent, GameEventType

from pokemon_legacy.displays.load_display import LoadDisplay
from pokemon_legacy.engine.general.controller import Controller
from pokemon_legacy.engine.general.Time import Time
from pokemon_legacy.engine.general.Route import Route
from pokemon_legacy.engine.general.utils import Colours, wait_for_key

# ======= Load displays =====
from pokemon_legacy.displays.game_display import GameDisplay, GameDisplayStates
from pokemon_legacy.displays.menu.menu_display_team import MenuTeamDisplay
from pokemon_legacy.engine.bag.menu_display_bag import MenuBagDisplay

from pokemon_legacy.engine.characters.character import Movement, Direction
from pokemon_legacy.engine.characters.npc import NPC, ProfessorRowan
...
from pokemon_legacy.engine.storyline.game_state import GameState, build_game_state_machine
from pokemon_legacy.engine.storyline.story_event import *
from pokemon_legacy.engine.storyline.story_events.choose_starter import *
...
pokedex = pd.read_csv("assets/data/pokedex/Local Dex.tsv", delimiter='\t', index_col=1)
...
class Game:
    def __init__(
        ...
        self.data_path: str = f"assets/data/save_states/{'save_state_' + str(save_slot) if not new else 'start'}"

        native_size = pg.Vector2(256, 382)
        self.graphics_scale = cfg.graphics_scale
        ...
        # ==== LOG INITIALISATION ====
        self.log, self.log_dir = GameLog(), "assets/data/logs"
    ...
    def _load_from_save_state(self):
        if self.cfg.save_slot:
            save_file = f"assets/data/save_states/save_state_{self.cfg.save_slot}/game_state.json"
        else:
            save_file = f"assets/data/save_states/start/game_state.json"
    ...
    def wait_for_key(
        ...
        :param key: pygame key reference
        :param break_on_timeout: If True, break after a set timeout
    ...
    def process_game_interaction(
        ...
        for action in interaction:
            if action.action_type == GameActionType.move:
                ...
                def perform_pan(start: pg.Vector2, target: pg.Vector2, duration: int, frames: int):
                    """Perform a single camera pan with easing."""
                    ...
                    for frame in range(frames):
                        ...
                        eased_t = action.apply_easing(t, action.easing)
                        
                        # Interpolate position using eased time
                        self.game_display.camera_offset = pg.Vector2(
                        ...
                        elapsed = pg.time.get_ticks() - frame_start
                        delay = max(0, int(frame_duration - elapsed))
                        pg.time.delay(delay)
                    
                    # Ensure exact final position
                    self.game_display.camera_offset = target
                
                # Calculate pan vectors
                start_offset = pg.Vector2(self.game_display.camera_offset)
                total_movement = action.direction.value * action.tiles
                target_offset = start_offset + total_movement
                
                # Perform the pan
                perform_pan(start_offset, target_offset, action.duration, action.frames)
                
                # Handle return to start if requested
                if action.return_to_start:
                    pg.time.delay(action.pause_at_target)
                    perform_pan(target_offset, start_offset, action.duration, action.frames)
            ...
            elif action.action_type == GameActionType.set_following_character:
                action: SetFollowingCharacter
                if action.following:
                    # Attach follower to player
                    self.player.set_follower(action.actor)
                    self.player.max_history_length = action.distance
                    
                    # Ensure follower is on the current map near the player
                    if self.game_display.map not in action.actor.map_positions:
                        player_pos = self.player.map_positions[self.game_display.map]
                        # Place follower one tile behind the player
                        follower_offset = self.player.facing_direction.value * -1
                        action.actor.map_positions[self.game_display.map] = player_pos + follower_offset
                    
                    # Add follower to map's object layer if not already there
                    self.game_display.map.add_character(
                        action.actor, 
                        position=action.actor.map_positions[self.game_display.map]
                    )
                else:
                    # Remove follower from player
                    self.player.clear_follower()
    ...
    def save(self):
        # save_temp = f"assets/data/save_states/save_{self.save_slot}_temp"
        save_dir = f"assets/data/save_states/save_state_{self.save_slot}"
        save_error = False
