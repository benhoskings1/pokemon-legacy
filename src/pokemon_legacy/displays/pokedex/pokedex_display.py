...
from pokemon_legacy.engine.general.utils import Colours, BlitLocation
from pokemon_legacy.engine.graphics.sprite_screen import SpriteScreen, DisplayContainer
from pokemon_legacy.engine.pokemon.pokemon import loader, Pokemon
...
class PokedexDisplayMain(SpriteScreen):
    ...
    def update(self):
        ...
        name = loader.old_pokedex.loc[loader.old_pokedex["ID"] == self.pokedex_display.pokemon_idx].index.values[0]
        ...
        for i in sorted(range(max([-(self.pokedex_display.pokemon_idx-1), -3]), min([4, -(self.pokedex_display.pokemon_idx - 152)])), key=lambda x: abs(x), reverse=True):
            name = loader.old_pokedex.loc[loader.old_pokedex["ID"] == self.pokedex_display.pokemon_idx+i].index.values[0]
...
class PokedexDisplayInfo(SpriteScreen):
    ...
    def update(self):
        ...
        if caught:
            ...
            img_path = f"assets/images/Type Labels/{pk_type} Label.png"
