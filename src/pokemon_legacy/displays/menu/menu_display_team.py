...
from pokemon_legacy.engine.general.utils import Colours, create_display_bar
from pokemon_legacy.engine.graphics.screen_V2 import FontOption, BlitLocation
...
class PokemonMovesContainer(DisplayContainer):
    ...
    def load_pokemon_details(self, pokemon: Pokemon):
        for idx, move in enumerate(pokemon.moves):
            ...
            # move type
            self.load_image(
                f"assets/images/Type Labels/{move.type} Label.png", pg.Vector2(31, 5 + 32*idx)*self.scale,
                scale=self.scale
            )
