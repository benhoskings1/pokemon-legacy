...
class ImageEditor:
    ...
    def createSurface(self, bgr=True):
        ...
        if self.pixelData.dtype != np.uint8:
            data = self.pixelData.astype(np.uint8)
        else:
            data = self.pixelData

        # OpenCV is BGR/BGRA, Pygame expects RGBA usually, or we can specify format
        # If bgr=True (default from cv2), we have BGRA
        # pg.image.frombuffer can handle "BGRA"
        ...
        if channels == 4:
            fmt = "BGRA" if bgr else "RGBA"
        elif channels == 3:
            fmt = "BGR" if bgr else "RGB"
        else:
            fmt = "P" # Palette/Greyscale? Unlikely to hit this path with cv2 imread

        # We need to ensure the array is C-contiguous for frombuffer
        if not data.flags['C_CONTIGUOUS']:
            data = np.ascontiguousarray(data)
            
        return pg.image.frombuffer(data.tobytes(), (width, height), fmt).convert_alpha()
