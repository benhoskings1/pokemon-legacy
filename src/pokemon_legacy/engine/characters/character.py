...
from pokemon_legacy.engine.storyline.game_action import GameAction
...
from pokemon_legacy.engine.graphics.sprite_set import SpriteSet2
...
class Character(GameObject):
    ...
    def __init__(self, properties: dict = None, scale: float = 1.0, map_scale: int | float = 1.0):
        ...
        # Follower tracking
        self.follower: Character | None = None  # Character following this one
        self.position_history: list[tuple] = []  # List of (map, position) tuples
        self.max_history_length: int = 1  # How many positions to track

        # load on instance creation
        self._load_surfaces()

    def record_position(self, game_map, position: pg.Vector2) -> None:
        """
        Record the current position before moving. Used for follower tracking.
        
        :param game_map: The map the character is on
        :param position: The position to record
        """
        self.position_history.append((game_map, pg.Vector2(position)))
    ...
    def get_last_position(self) -> tuple | None:
        ...
        :return: Tuple of (map, position) or None if no history
        """
