...
from pokemon_legacy.engine.game_world.tiled_map import TiledMap2, GameObject, MapLinkTile
from pokemon_legacy.maps.buildings.pokemart.pokemart_containers import (
    MoneyContainer, ItemSetContainer, DisplayBar, BagCountContainer, PriceCounter,
    ConfirmContainer, ConfirmOption
)

from pokemon_legacy.engine.general.item import ItemGenerator
...
class ActionSelector(DisplayContainer):
    def __init__(self, scale=1.0):
        image_path = os.path.join(ASSET_PATH, "maps/buildings/pokemart/assets/desk_popup_1.png")
...
class PokeMart(TiledMap2, MapLinkTile, StateMachine):
    ...
    with open(os.path.join(ASSET_PATH, 'data/game_config/pokemart_data.json'), 'r') as file:
    ...
    def __init__(self, rect, player, map_scale=1, obj_scale=1, parent_map_scale=1.0, properties=None):
        ...
        TiledMap2.__init__(
            self,
            os.path.join(ASSET_PATH, "maps/buildings/pokemart/pokemart.tmx"),
            size,
            player,
